...
const taskStatusEnum = readStringEnum(
  taskSchema,
  ["properties", "status", "enum"],
  "task.status schema enum",
);
...
function validateTransitionRequest(request) {
  if (!request.nextStatus || request.nextStatus.startsWith("--")) {
    errors.push(
      `missing task status for --validate-transition; expected ${formatTaskStatusEnum()}`,
    );
...
function validateTaskStatus(taskFile, status) {
  if (!status) {
    errors.push(
      `${toRepoPath(taskFile)}: missing task status; expected ${formatTaskStatusEnum()}.`,
    );
    return;
  }
  if (!taskStatusEnum.has(status)) {
    errors.push(
      `${toRepoPath(taskFile)}: invalid task status "${status}"; expected ${formatTaskStatusEnum()}.`,
    );
  }
}
