...
1. **Spec drift**
   - code changes but contracts do no
2. **Task inflation**
...
- **Vision and mission**: optional, freeform direction context that is present only when a repository adopts i
- **Roadmap Item**: an addressable strategic bet grouped by `now`, `next`, or `later
- **Outcome**: measurable business resul
...
- local markdown truth in `.projec
...
### 6.8 Optional strategy-layer contrac
...
### 7.1 Why compact runtime states exis
...
- no production merge directly from probe outpu
...
- define a measurable outcome, create or review the planned Spec, and make the probe decision explici

**Entry criteria**

- problem and owner identified
- imported or researched intent, if present, has been reviewed
- when the project spec has `roadmap_item`, that roadmap item has been resolved and read as strategic source contex
...
- uncertainty rated and probe decision explici
...
- touched surfaces and footguns explici
...
2. dependency sequencing is clear upfron
...
1. announce stream scope at star
...
- merge and release control poin
...
- path privacy enforcemen
- hidden/bidirectional Unicode control enforcemen
- GitHub remote safety checks

### 14.2 Default team policy pack

1. one outcome per active delivery project scope
2. one canonical spec per active projec
...
9. repository structure and naming remain agent-readable by defaul
...
### 16.0 Roles are hats, not headcoun
...
#### Decomposition checklis

- [ ] each task is atomic
- [ ] each task has owner and estimate
- [ ] dependency graph acyclic
- [ ] conflict hotspots explici
...
- legacy `ready` remains valid; migrate it to `planned` only when the task is not actually executable ye
- `in-progress` -> `in-progress
- `closed` -> `done
...
- no active task is los
- all active mappings preserved
- status parity maintained
- old artifacts remain readable for audi
...
- two end-to-end dry runs with no unresolved drif
...
- one multi-stream delivery completed without uncontrolled merge conflic
...
### 4.3 Alternatives and why they are not defaul
