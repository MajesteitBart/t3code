...
The planning bootstrap is complete. Separate 2026-08-08 delivery requests authorized WS-A, WS-B, the WS-E T-017 dependency enabler, WS-C, and dependency-safe WS-D work. Those scopes now provide the independently buildable application, canonical contracts and policy-free transport, protected persistence, collision-safe state/reconciliation, active/passive recovery, stable ambiguous-turn handling, a real disposable two-server fixture, and the thin onboarding/environment/project/thread Compose shell. T-016 remains gated by WS-E T-018; production chat/composer and workspace parity remain follow-on work rather than foundation scope.
