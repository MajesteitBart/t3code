...
- Native Android WS-A, WS-B, and WS-C are done, WS-D T-014/T-015 are done, and WS-E's T-017 disposable real-server harness is complete. D-015 allows later adapter-first review-diff/composer work, defers the Ghostty/JNI terminal pending its native supply-chain gates, and selects Compose-native replacement for Expo-specific controls.
- The native app builds debug/release-shaped variants and renders a masked onboarding plus collision-safe aggregate environment/project/thread shell on supported API-35 phone and expanded tablet profiles. Its Kotlin boundary covers canonical contracts, policy-free HTTP/RPC attempts, protected credentials, Room-backed multi-environment state, active/passive supervision, stale-publication guards, process-death recovery, stable ambiguous-turn identity, and lifecycle-aware UI composition. The disposable harness verifies two real isolated T3 servers, durable receipts/events, restart, revocation, and exact teardown. T-018 performance evidence gates T-016 accessibility/lifecycle completion; CI, release, push-provider, chat/composer, and workspace scope remain later work.
