...
- Native Android WS-A and WS-B are done. D-015 allows later adapter-first review-diff/composer work, defers the Ghostty/JNI terminal pending its native supply-chain gates, and selects Compose-native replacement for Expo-specific controls.
- The native app builds debug/release-shaped variants with collision-safe identities and renders on the supported API-35 phone emulator. Its Kotlin protocol boundary now conforms to canonical fixtures and provides direct pairing/HTTP plus a fresh-ticket, one-attempt Effect RPC session with deterministic race and compression evidence. Later workstreams still own persistence/supervision, product shell, real-server integration, CI, release, and push-provider scope.
